...
def _build_prompt_messages(
    ...
    *,
    ...
    system_prompt = (
        ...
        "  절대 빈 배열 금지)를 포함합니다.\n"
        "- rawLineText는 모든 CREATE/UPDATE/DELETE item에 반드시 존재하는 비어 있지 않은 문자열입니다."
        " 전체 사용자 메시지를 모든 item에 동일하게 복사하지 말고, 사용자 원문 중 각 item에"
        " 대응하는 부분만 그대로 넣으세요.\n"
        ...
        " 아니라, 문맥을 이해해 자연스러운 작업명으로 정규화하라는 뜻입니다. 예: \"자료구조"
        " 과제 3문제를 풀어야 해\"는 title=\"자료구조 과제\", amountText=\"3문제\"로 분리하고"
        " title에 \"3문제\"를 남기지 않습니다.\n"
        "- 반대로 숫자가 분량이 아니라 그 작업을 다른 것과 구분 짓는 고유 정보(예: \"2차"
        " 과제\", \"3분 스피치\", \"운영체제 과제 2번\", \"문제 3 풀이\")면 title에서 지우지"
        " 않습니다. 지우면 오히려 다른 과제와 구분이 안 됩니다.\n"
        "- 숫자가 분량인지 고유 식별 정보인지 문맥만으로 확정할 수 없으면 title에서 임의로"
        " 빼거나 amountText로 옮기지 말고, 위에서 정의한 pendingQuestion 흐름으로 확인"
        " 질문을 하세요(title 또는 amount를 missing으로 두고 되묻습니다 — 새 필드를 만들지"
        " 않습니다).\n"
        "- 이 title/amountText 분리 규칙은 action=\"CREATE\"뿐 아니라 action=\"UPDATE\"로"
        " title을 새로 채울 때도(updateFields에 \"title\"이 있을 때) 동일하게 적용됩니다.\n\n"
        ...
        " 거부).\n"
        "- TASK CREATE: rawLineText, entityType, action, targetEntityId, deadlineState,"
        " normalizedPayload, pendingQuestion이 모두 필수입니다.\n"
        "- TASK UPDATE: rawLineText, entityType, action, targetEntityId, deadlineState,"
        " normalizedPayload, pendingQuestion, updateFields가 모두 필수입니다.\n"
        "- TASK DELETE: rawLineText, entityType, action, targetEntityId, normalizedPayload,"
        " pendingQuestion이 모두 필수입니다.\n"
        "- FIXED_SCHEDULE: CREATE/UPDATE/DELETE 어떤 action이든 deadlineState를 절대 포함하지"
        " 마세요(TASK 전용 키입니다).\n\n"
        "[응답 전 self-check — 응답을 만들기 직전 아래 6가지를 순서대로 다시 확인하세요]\n"
        ...
        "5-1. 모든 item에 rawLineText가 있고 공백이 아닌지, 그 값이 전체 메시지 복사가 아니라"
        " 사용자 원문 중 해당 item에 대응하는 부분인지 확인하세요.\n"
        "6. TASK CREATE/UPDATE에서 title을 채웠다면(UPDATE는 updateFields에 \"title\"이 있을"
        " 때만) 그 title에 amountText/deadlineAt/estimatedMinutes/remainingMinutes로 이미"
        " 분리한 분량·마감·시간 표현이 중복으로 남아있지 않은지 다시 확인하세요. 단, 지금"
        " 지우려는 숫자가 실제로는 그 작업을 구분 짓는 고유 번호나 제목의 일부라면 지우지"
        " 마세요.\n\n"
        ...
        "[예시 3 — title에는 amountText로 옮긴 분량을 중복하지 않는다]\n"
        "입력 \"자료구조 과제 3문제를 수요일까지 풀어야 해\" → title=\"자료구조 과제\","
        " amountText=\"3문제\"(금지: title=\"자료구조 과제 3문제 풀기\").\n"
        "입력 \"영단어 30개 외우기\" → title=\"영단어 암기\", amountText=\"30개\""
        "(금지: title=\"영단어 30개 외우기\").\n"
        "입력 \"책 50페이지 읽기\" → title=\"책 읽기\", amountText=\"50페이지\""
        "(금지: title=\"책 50페이지 읽기\").\n"
        "입력 \"운영체제 과제 2번 제출하기\"에서 \"2번\"이 분량이 아니라 과제를 구분하는"
        " 고유 번호라면 title=\"운영체제 과제 2번 제출\"로 그대로 두고 amountText는"
        " null입니다(숫자를 지우면 다른 과제와 구분이 안 됨).\n"
        "입력에 있는 숫자가 분량인지 고유 번호인지 문맥만으로 알 수 없으면 title이나"
        " amountText로 단정하지 말고 pendingQuestion으로 확인 질문을 하세요."
    )
...
def _build_repair_messages(
    ...
    *,
    ...
    repair_instruction = (
        ...
        f"{violation_code}). 직전 응답을 완전히 대체하는 올바른 JSON 하나만 다시 출력하세요. "
        "기존의 유효한 필드를 제거하거나 action을 변경하지 말고, 보고된 계약 위반 필드를 "
        "교정하세요. 설명·마크다운·코드블록 없이 JSON 객체만 응답하세요."
    )
    if "MISSING_REQUIRED_KEY:rawLineText" in violation_code:
        repair_instruction += (
            " 누락된 해당 action item에 rawLineText를 반드시 추가하세요. rawLineText는 TASK "
            "CREATE/UPDATE/DELETE 모두에서 필수이며, 특히 TASK UPDATE에서도 deadlineState와 "
            "updateFields뿐 아니라 반드시 포함해야 합니다. 값은 비어 있지 않은 문자열이어야 "
            "하고, 전체 사용자 메시지를 모든 item에 동일하게 복사하지 말고 사용자 원문 중 해당 "
            "item에 대응하는 부분을 넣으세요."
        )
...
def _build_unresolved_answer_prompt_messages(
    ...
    *,
    ...
    system_prompt = (
        ...
        " CREATE/UPDATE/DELETE item 규칙을 아래의 현재 prompt 내 exact-key 규칙대로 따릅니다.\n"
        "- TASK UPDATE exact keys: entityType, action, targetEntityId, rawLineText, deadlineState,"
        " normalizedPayload, pendingQuestion, updateFields. 여덟 키를 모두 포함하고 다른 키는"
        " 포함하지 마세요. updateFields는 비어 있지 않은 배열이어야 합니다.\n"
        "- TASK CREATE/DELETE 역시 rawLineText가 필수입니다. CREATE는 entityType, action,"
        " targetEntityId, rawLineText, deadlineState, normalizedPayload, pendingQuestion을, DELETE는"
        " entityType, action, targetEntityId, rawLineText, normalizedPayload, pendingQuestion을 정확히"
        " 포함합니다. FIXED_SCHEDULE은 기존 계약대로 deadlineState를 포함하지 않습니다.\n"
        "- 모든 item의 rawLineText는 반드시 비어 있지 않은 문자열이어야 합니다. 전체 사용자"
        " 메시지를 기계적으로 복사하지 말고, 사용자 원문 중 해당 item에 대응하는 부분을"
        " 넣으세요.\n"
