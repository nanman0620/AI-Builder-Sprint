...
def test_completed_block_is_protected_from_deletion():
    user_id, cycle_id, db = _setup()
    task = make_task(user_id=user_id, plan_cycle_id=cycle_id, remaining_minutes=0, created_at=_now())
...
def test_not_done_block_is_protected_from_deletion():
    user_id, cycle_id, db = _setup()
    task = make_task(user_id=user_id, plan_cycle_id=cycle_id, remaining_minutes=40, created_at=_now())
...
def test_task_with_current_period_checked_block_skips_current_period_and_uses_effective_remaining():
    user_id, cycle_id, db = _setup()
    task = make_task(user_id=user_id, plan_cycle_id=cycle_id, remaining_minutes=150, created_at=_now())
    checked = make_plan_block(
        user_id=user_id,
        plan_cycle_id=cycle_id,
        task_id=task.id,
