...
| `UI-005`~`UI-007` | 홈 우측 상단의 상점 진입 | 이 범위의 `HomeHeader`에 상점 PNG를 표시하고 누르면 MVP 미지원 안내 모달만 연다. 상점 Route, API, 재화, 구매와 화면 이동은 구현하지 않음 |
| `UI-008`, `UI-009` | 상점 아이콘 없음(원본 캡처 기준) | 2026-08-02 사용자 결정: `DEADLINE_WARNING`의 `HomeHeader`는 상점 아이콘을 표시하지 않는다(원본 캡처와 일치). `UI-005`~`UI-007`의 상점 아이콘 규칙과는 예외로 분리 |
